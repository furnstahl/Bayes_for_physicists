...
function Visualizer(canvas, xHistCanvas, yHistCanvas) {
  ...
  this.nutsColor       = '#09c';
  this.contourColor    = '#25e';
  this.sampleDrawFill  = '#999';

  this.histogramRatio = 0.2;
  this.histBins = 50;
  this.histFillStyle   = '#25e';
...
Visualizer.prototype.drawHistograms = function(options) {
  ...
  context.lineWidth = 2 * window.devicePixelRatio * this.lw;
...
Visualizer.prototype.drawCircle = function(canvas, options) {
  ...
  context.lineWidth = (options.lw) ? options.lw * window.devicePixelRatio * this.lw  : 1 * window.devicePixelRatio * this.lw ;
...
Visualizer.prototype.drawSample = function(canvas, center) {
  var context = canvas.getContext('2d');
  context.globalCompositeOperation = 'multiply';
  this.drawCircle(canvas, { fill: this.sampleDrawFill, center: center, radius: 0.02 * this.lw , lw: 0});
  context.globalCompositeOperation = 'source-over';
};
...
Visualizer.prototype.dequeue = function() {
  ...
  if (event.type == 'proposal') {
    ...
    if (event.hasOwnProperty('initialMomentum')) {
      ...
      this.drawArrow(this.overlayCanvas, {from: last, to: to, color: this.proposalColor, lw: this.lw });
    }
    ...
    if (event.hasOwnProperty('trajectory')) {
      if (this.animateProposal) {
        ...
        this.drawPath(this.overlayCanvas, { path: event.trajectory, color: this.trajectoryColor, lw: this.lw });
        for (var i = 0; i < event.trajectory.length - 1; ++i) {
          this.drawCircle(this.overlayCanvas, { fill: this.trajectoryColor, center: event.trajectory[i], radius: 0.02 , lw: 0});
        }
        this.drawArrow(this.overlayCanvas, {from: event.trajectory[event.trajectory.length - 2], to: event.trajectory.last(), color: this.trajectoryColor, lw: this.lw });
      }
    ...
    if (event.hasOwnProperty('nuts_trajectory')) {
      ...
      if (this.animateProposal) {
        ...
        for (var i = 0; i < event.nuts_trajectory.length; ++i) {
          ...
          this.drawPath(this.overlayCanvas, { path: [event.nuts_trajectory[i].from, event.nuts_trajectory[i].to], color: color, lw: this.lw });
    ...
    if (event.hasOwnProperty('ns_rejected')) {
      ...
      if (this.animateProposal) {
        ...
        for (var i = 0; i < event.ns_rejected.length; ++i) {
           this.drawArrow(this.overlayCanvas, { from: event.previous, to: event.ns_rejected[i], color: this.rejectColor, lw: this.lw*2 });
    ...
    if (event.hasOwnProperty('gradient')) {
      this.drawArrow(this.overlayCanvas, { from: last, to: last.add(event.gradient), color: this.nutsColor, lw: this.lw });
      this.drawArrow(this.overlayCanvas, { from: last.add(event.gradient), to: event.proposal, color: this.proposalColor, lw: 1.5*this.lw });
    ...
    if (event.hasOwnProperty('proposalCov') && drawProposalCov) {
      ...
      if (event.hasOwnProperty('proposalMean')) {
        drawProposalArrow = false;
        this.drawPath(this.overlayCanvas, { path: [last, center], color: this.proposalColor, lw: this.lw });
        this.drawArrow(this.overlayCanvas, { from: center, to: event.proposal, color: this.proposalColor, lw: this.lw });
      }
    }
    ...
    if (event.hasOwnProperty('revProposalCov') && drawProposalCov) {
      ...
      if (event.hasOwnProperty('revProposalMean')) {
        drawProposalArrow = false;
        this.drawPath(this.overlayCanvas, { path: [event.proposal, event.revProposalMean], color: '#00f', lw: this.lw });
        this.drawArrow(this.overlayCanvas, { from: center, to: last, color: '#00f', lw: this.lw });
      }
    }

    // draw proposal arrow
    if (drawProposalArrow) {
      this.drawArrow(this.overlayCanvas, {from: last, to: event.proposal, color: this.proposalColor, lw: this.lw });
    }
  }
  ...
  if (event.type == 'trajectory-animation-step') {
    ...
    this.drawPath(this.overlayCanvas, { path: path, color: this.trajectoryColor, lw: this.lw});
    ...
    this.drawCircle(this.overlayCanvas, { fill: this.trajectoryColor, center: event.trajectory[event.offset + 1], radius: 0.02 * this.lw * 2 , lw: 0});
  }
  ...
  if (event.type == 'ns-trajectory-animation-step') {
    this.tweening = true; // start skiping delay for calling requestAnimationFrame
    this.drawArrow(this.overlayCanvas, { from: event.previous, to: event.ns_rejected[event.offset], color: this.rejectColor, lw: 2*this.lw });
  ...
  if (event.type == 'nuts-animation-step') {
    ...
    if (type == 'accept' || type == 'reject') {
      ...
      this.drawPath(this.overlayCanvas, { path: path, color: color, lw: (type == 'accept') ? this.lw : 0.5*this.lw });
      this.drawCircle(this.overlayCanvas, { color: color, center: event.trajectory[event.offset].to, radius: 0.02, lw: 0.5*this.lw });
  ...
  if (event.type == 'accept') {
    this.drawArrow(this.overlayCanvas, { from: last, to: event.proposal, color: this.acceptColor, lw: 2*this.lw });
    this.drawSample(this.samplesCanvas, event.proposal);
    this.drawHistograms();
  }

  if (event.type == 'reject') {
    this.drawArrow(this.overlayCanvas, { from: last, to: event.proposal, color: this.rejectColor, lw: 2*this.lw });
  ...
  if (event.type == 'radfriends-region') {
    ...
    context.clearRect(0, 0, this.canvas.width, this.canvas.height);
    for (var i = 0; i < event.x.length; i++) {
      this.drawCircle(this.overlayCanvas, { fill: '#cfc', color: '#afa', center: event.x[i], radius: event.r, lw: this.lw});
      //context.beginPath();
    ...
    for (var i = 0; i < event.x.length; i++) {
      this.drawCircle(this.overlayCanvas, { fill: '#00f', center: event.x[i], radius: 0.02, lw: this.lw});
    }
  }
  if (event.type == 'ns-dead-point') {
    this.drawArrow(this.overlayCanvas, { from: event.deadpoint, to: event.proposal, color: this.acceptColor, lw: 2*this.lw });
    this.drawSample(this.samplesCanvas, event.proposal);

    //var context = this.overlayCanvas.getContext('2d');
    //context.globalCompositeOperation = 'multiply';
    for (var i = 0; i < event.rejected.length; ++i) {
      this.drawCircle(this.overlayCanvas, { fill: this.rejectColor, color: this.rejectColor, center: event.rejected[i], radius: 0.02, lw: 3*this.lw});
      //this.drawArrow(this.overlayCanvas, { from: event.previous, to: event.rejected[i], color: this.rejectColor, lw: 2 });
      //this.drawSample(this.samplesCanvas, event.ns_rejected[i]);
    }
    this.drawCircle(this.overlayCanvas, { fill: this.acceptColor, color: this.acceptColor, center: event.proposal, radius: 0.02, lw: 3*this.lw});
    //context.globalCompositeOperation = 'source-over';
...
Visualizer.prototype.drawDensityContours = function() {
  ...
  for (var i = 0; i < this.simulation.mcmc.contours.length; ++i) {
    ...
    this.drawPath(this.densityCanvas, {path:this.simulation.mcmc.contours[i], color: this.contourColor, alpha: alpha, lw: this.lw });
  }
