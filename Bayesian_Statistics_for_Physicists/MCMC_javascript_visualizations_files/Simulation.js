...
window.onload = function() {
  ...
  sim.mcmc.leapfrogSteps = leapfrogSteps;
  window.onresize = function() { viz.resize(); };
  
  gui = new dat.GUI({width: 300});

  gui.add(sim, 'delay', 0, 1000).name('Animation delay').onChange(function(value) {
    if (value == 0) {
  ...
  //gui.add(sim, 'tweeningDelay', 0, 200).name('Tweening delay');
  gui.add(sim, 'reset').name('Reset simulation');
